...
class ContactsRepository:
    ...
    async def get_contact_by_id(self, cnt_id: int) -> Contact | None:
        stmt = select(Contact).filter_by(id=cnt_id)
    ...
    def __get_upcoming_birthday_stmt(self, start_date: date, end_date: date):
        """Add filter of the birthdays for statement"""
        return select(Contact).filter(
            or_(
                and_(
    ...
    async def get_contacts_upcoming_birthdays(self, days: int, limit: int, offset: int):
        ...
        if end_date > year_end:
            stmt_current_year = self.__get_upcoming_birthday_stmt(
                today, year_end
            )
            stmt_next_year = self.__get_upcoming_birthday_stmt(
                next_year, end_date
            )
            stmt = select(
                aliased(
            ...
            stmt = self.__get_upcoming_birthday_stmt(today, end_date)
